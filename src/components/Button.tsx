...
const Button: React.FC<Props> = ({
  ...
  const buttonStyle = {
    ...
    fontSize: "29.6px",
    color: "white",
    backgroundColor: "#4AC1F3",
    transition: "background-color 0.5s ease",
    border: "none",
    animation: clicked ? "button-click 0.6s" : "none",
  };

  const animationKeyframes = `
    @keyframes button-click {
      0% { background-color: #4AC1F3; }
      50% { background-color: #0072a3d0; }
      100% { background-color: #4AC1F3; }
    }
  `;
  ...
  return (
    <>
      <button
        style={{ ...buttonStyle, width: `${width}px`, height: `${height}px` }}
        onClick={handleClick}
